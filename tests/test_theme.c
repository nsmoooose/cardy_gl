...
START_TEST(test_theme_list) {
	...
	themes *t = theme_list(context, "resources/card_decks/french/themes");

	ck_assert_msg(t != 0, "No themes struct returned.");
	ck_assert_msg(t->theme_count == 7, "Incorrect number of themes listed.");
	...
	for (i = 0; i < 7; ++i) {
		ck_assert_msg(strcmp(t->theme_names[i], "ancient_french") == 0 ||
		                  ...
		                  strcmp(t->theme_names[i], "ornamental") == 0 ||
		                  strcmp(t->theme_names[i], "paris") == 0 ||
		                  strcmp(t->theme_names[i], "plastic") == 0,
