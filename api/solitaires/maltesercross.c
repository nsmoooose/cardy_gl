	...
	if (i->state == 1) {
		...
		*/
		...
		if (card_count(i->deck) == 0) {
			...
			card_reveal_all_array(5, i->pile1, i->pile2, i->pile3, i->pile4,
			                      i->pile5);
		}
	} else if (i->state == 2) {
		if (card_count(i->deck) == 0) {
...
static void setup_rules(mem_context *context, solitaire *s, internal *i) {
	...
	king_pile_cond = condition_destination_array(context, 4, i->king1, i->king2,
	                                             i->king3, i->king4);

	source_not_done = condition_source_array(
		context, 18, i->center, i->king1, i->king2, i->king3, i->king4,
		i->build1, i->build2, i->build3, i->build4, i->pile1, i->pile2,
		i->pile3, i->pile4, i->pile5, i->src1, i->src2, i->src3, i->src4);
	...
	rule_add_condition(context, rule5,
	                   condition_destination(context, i->center));
	...
	rule_add_condition(context, rule6,
	                   condition_destination(context, i->center));
	...
	rule_add_condition(context, rule7,
	                   condition_source_array(context, 4, i->king1, i->king2,
	                                          i->king3, i->king4));
...
solitaire *solitaire_maltesercross(mem_context *context,
	...
	src1->origin[1] =
		settings->card_height + settings->card_spacing + separator;
	visual_add_pile(context, s->visual, src1);

	i->src2 = pile_create(context, 13);
	src2 = visual_pile_create(context, i->src2);
	src2->origin[1] =
		0 - (settings->card_height + settings->card_spacing) + separator;
	...
	build3->origin[1] = settings->card_height / 2 + settings->card_spacing * 2 +
						settings->card_height + settings->card_width / 2 +
						separator;
	...
	build4->origin[1] =
		0 -
		(settings->card_height / 2 + settings->card_spacing * 2 +
	     settings->card_height + settings->card_width / 2) +
		separator;
	...
	pile1->origin[0] =
		0 - (settings->card_spacing * 2 + settings->card_width * 2);
