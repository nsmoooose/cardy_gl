...
static void action_deal_execute(visual_pile_action *action) {
	...
	if (card_count(i->deck) == 104) {
		...
		card_append(card_take_match(i->deck, card_match_value, &prototype),
		            i->kings[0]);
		...
		card_move_all_array(
			i->deck, 24, i->kings[0], i->kings[1], i->kings[2], i->kings[3],
			...
			i->aces[0], i->aces[1], i->aces[2], i->aces[3], i->aces[4],
			i->aces[5], i->aces[6], i->aces[7]);
...
static void setup_rules(mem_context *context, solitaire *s, internal *i) {
	...
	condition *src = condition_source_array(
		context, 16, i->kings[0], i->kings[1], i->kings[2], i->kings[3],
		i->kings[4], i->kings[5], i->kings[6], i->kings[7], i->aces[0],
		i->aces[1], i->aces[2], i->aces[3], i->aces[4], i->aces[5], i->aces[6],
		i->aces[7]);

	condition *dst_ace = condition_destination_array(
		context, 8, i->aces[0], i->aces[1], i->aces[2], i->aces[3], i->aces[4],
		i->aces[5], i->aces[6], i->aces[7]);

	condition *dst_king = condition_destination_array(
		context, 8, i->kings[0], i->kings[1], i->kings[2], i->kings[3],
		i->kings[4], i->kings[5], i->kings[6], i->kings[7]);
	...
	rule_add_condition(context, r,
	                   condition_source_not_equal_destination(context));
	rule_add_condition(context, r,
	                   condition_destination_card_equal(context, e_suit_none,
	                                                    13, e_equal_value, 0));
	rule_add_condition(context, r,
	                   condition_not(context, condition_source_card_equal(
												  context, e_suit_none, 13,
												  e_equal_value, 0)));
	rule_add_condition(
		context, r,
		condition_not(context, condition_source_card_equal(
	...
	rule_add_condition(
		...
		condition_card_count_array(context, 11, 8, i->aces[0], i->aces[1],
	                               i->aces[2], i->aces[3], i->aces[4],
	                               i->aces[5], i->aces[6], i->aces[7]));
}
...
solitaire *solitaire_heirship(mem_context *context, visual_settings *settings) {
	...
	left = 0 - settings->card_spacing / 2 - settings->card_width * 4 -
		   settings->card_spacing * 4;
	for (index = 0; index < 8; ++index) {
		left_tmp =
			left + (settings->card_width + settings->card_spacing) * index;
